...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
        <Link to='/'><img src={assets.trendify} className='w-36' alt='Logo' /></Link>
        ...
        <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
            <div className='flex flex-col text-gray-600'>
                ...
                <NavLink onClick={() => setVisible(false)} className='py-2 text-center border' to='/'>HOME</NavLink>
                <NavLink onClick={() => setVisible(false)} className='py-2 text-center border' to='/collection'>COLLECTION</NavLink>
                <NavLink onClick={() => setVisible(false)} className='py-2 text-center border' to='/about'>ABOUT</NavLink>
                <NavLink onClick={() => setVisible(false)} className='py-2 text-center border' to='/contact'>CONTACT</NavLink>
            </div>
        </div>
    </div>
  )
